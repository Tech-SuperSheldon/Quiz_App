...
export default function RecentQuizActivityCard() {
  ...
  return (
    <Card className="h-full flex flex-col border-slate-100/80 bg-white/90 shadow-md shadow-slate-200/80">
        ...
        {loading ? (
          ...
          <ul className="space-y-3">
            {activities.map((t, index) => (
              <li
                key={index}
                className="flex items-center justify-between p-2.5 hover:bg-slate-50 rounded-lg transition-colors border border-transparent hover:border-slate-100"
              >
                <div>
