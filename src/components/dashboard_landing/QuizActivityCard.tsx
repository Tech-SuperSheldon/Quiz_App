  ...
  return (
    <Card className="border-slate-100/80 bg-white/90 shadow-md shadow-slate-200/80">
      ...
      <CardContent className="pt-0">
        ...
        <div className="mt-1 flex items-end gap-3 h-20">
          {loading ? (
            Array(7)
              .fill(0)
              .map((_, i) => (
            ...
            activityData.map((data, i) => {
              const heightPercent =
                data.count === 0 ? 10 : (data.count / maxCount) * 100;
              ...
              return (
                ...
                >
                  <div className="opacity-0 group-hover:opacity-100 transition-opacity text-[10px] bg-slate-800 text-white px-1.5 py-0.5 rounded absolute -top-5">
                    {data.count}
                  </div>
        ...
        <div className="mt-2 text-sm text-slate-500">
          Quizzes taken per day (last 7 days)
        </div>
      </CardContent>
    </Card>
  );
