...
import { AcademicCapIcon, TrophyIcon, BookOpenIcon, SparklesIcon } from "@heroicons/react/24/outline";
    ...
    <div className="space-y-6"> {/* Reduced from space-y-8 */}
      ...
      >
        <div className="flex items-center gap-2 mb-1"> {/* Reduced gap and margin */}
          <SparklesIcon className="h-6 w-6 text-yellow-500 animate-pulse" /> {/* Smaller icon */}
          <h1 className="text-2xl font-bold bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent">
            Welcome Back 👋
          </h1>
        </div>
        <p className="text-sm text-gray-600 dark:text-gray-300 max-w-2xl"> {/* Smaller text */}
          Continue your learning journey with personalized quizzes and interactive lessons tailored just for you.
        </p>
      </motion.div>
      ...
      >
        {sections.map((section, i) => (
          ...
          >
            ...
            <div className={`absolute inset-0 ${section.gradient} opacity-0 group-hover:opacity-5 blur-xl transition-opacity duration-500`}></div>
            ...
            <div className="relative p-4 rounded-xl bg-white/90 dark:bg-slate-800/90 backdrop-blur-lg border border-gray-200/50 dark:border-slate-700/50 shadow-lg hover:shadow-xl transition-all duration-500">
              ...
              <div className="flex items-center justify-between mb-3">
                <div className={`p-2 rounded-lg bg-gradient-to-r ${section.color} shadow-md`}> {/* Smaller padding */}
                  <section.icon className="h-5 w-5 text-white" /> {/* Smaller icon */}
                </div>
                <div className="text-right">
                  <span className="text-lg font-bold text-gray-800 dark:text-white"> {/* Smaller text */}
                    {section.progress}%
                  </span>
                  <div className="w-12 h-1 bg-gray-200 dark:bg-slate-700 rounded-full mt-1 overflow-hidden"> {/* Smaller progress bar */}
                    <div 
      ...
      >
        ...
        <div className="relative bg-gradient-to-r from-indigo-600 to-purple-500 text-white py-4 px-6 rounded-xl text-center font-semibold shadow-xl"> {/* Smaller padding */}
          <div className="flex items-center justify-center gap-2">
            <SparklesIcon className="h-5 w-5 animate-bounce" /> {/* Smaller icons */}
            <span className="text-base">Ready to learn? Start a quiz now!</span> {/* Smaller text */}
            <SparklesIcon className="h-5 w-5 animate-bounce delay-150" />
          </div>
          <p className="text-indigo-100 mt-1 text-xs font-normal"> {/* Smaller text */}
